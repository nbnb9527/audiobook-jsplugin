# -*- coding: utf-8 -*-
# 免责声明：脚本仅用于个人学习自用，仅做插件功能强化，无任何绕过授权、破解鉴权的改动；
# MiMusic 插件版权归 MiMusic Team 所有，禁止对外分发修改产物，使用产生的一切风险由使用者自行承担。
"""
...
PLUGIN_VERSION = os.environ.get("PLUGIN_VERSION") or "1.3.71"
...
def patch_ui(src: str) -> str:
    ...
    p9b_new = ('let i=t.sortBy||"updatedAt",d=t.order==="asc"?1:t.order==="desc"?-1:0,'
               ...
               'i==="title"?a.sort((c,u)=>{var A=__T(c),B=__T(u),p=A.match(/\\d+|\\D+/g)||[A],q=B.match(/\\d+|\\D+/g)||[B],m=Math.min(p.length,q.length),r=0;for(var i0=0;i0<m;i0++){var x=p[i0],y=q[i0];if(/^\\d+$/.test(x)&&/^\\d+$/.test(y)){var nx=+x,ny=+y;if(nx!==ny){r=nx<ny?-1:1;break}}else if(x!==y){r=x<y?-1:1;break}}if(!r)r=p.length-q.length;return r*(d||1)})'
    ...
    if not globals().get("SKIP_P8L", False):
        ...
        p8l_new = p8l_old + (
            ...
            'let e=k(o.query||""),kw=(e.keyword||"").trim().toLowerCase(),fav=e.favoritesOnly==="true",lf=e.libFilter||"",'
            ...
            'if(fav){var s2=new Set(t.settings.favorites||[]);a=a.filter(c=>s2.has(c.id))}if(lf==="shorts")a=a.filter(c=>c.virt==="shorts");else if(lf==="custom")a=a.filter(c=>c.virt==="custom");else if(lf==="pack")a=a.filter(c=>c.virt==="pack");else if(lf==="fav"){var s3=new Set(t.settings.favorites||[]);a=a.filter(c=>s3.has(c.id))}'
...
s.get("/api/similar-execute-progress",async()=>{let j=t.__simJob;return f({success:!0,data:j?{total:j.total,done:j.done,current:j.current,currentGroup:j.currentGroup,totalGroups:j.totalGroups,errors:j.errors,executed:j.execArr,finished:j.finished,startedAt:j.startedAt}:null})}),'''
        sim_routes_old = 's.post("/api/advanced-cache/clear",async o=>{'
...
let adv=!1,abt=!1;try{let su=await whNt("/mina/status?account_id="+c.accountId+"&device_id="+r,n),hd={};s&&(hd.Authorization="Bearer "+s);hd["Content-Type"]="application/json";let rp=await fetch(su,{headers:hd});if(!rp.ok)return;let rj=await rp.json();if(!rj.success||!rj.data)return;let dt=rj.data,m=dt.position??0,pl=dt.is_playing??dt.state==="playing",idl=dt.state==="idle";if(pl&&m>3)c.playing=!0;if(!pl&&m>=l*0.95){adv=!0;songloft.log.info("[auto-next] pos-based end: pos="+m+"/"+l+"s played="+pl+" idle="+idl+" -> advance");whLog("speaker","检测到章节结束 (pos="+Math.round(m)+"s/"+l+"s)，推送下一章",c.bookTitle+" #"+c.chapterIndex,null)}else if(!pl&&c.playing&&m<l*0.9){abt=!0;songloft.log.info("[auto-next] user interrupted at pos="+m+"/"+l+"s, abort auto-next");whLog("speaker","已手动停止，取消自动连播",c.bookTitle+" #"+c.chapterIndex,null)}else{console.log("[auto-next] status: pos="+m+"/"+l+"s played="+pl+" idle="+idl);let t0=whQ[c.bookId+"_"+r];if(t0){let el=(Date.now()-t0)/1e3;if(c.playing&&el>=l+1){adv=!0;songloft.log.info("[auto-next] timer-based: elapsed="+Math.round(el)+"s dur="+l+"s -> advance");whLog("speaker","计时判定章节结束 ("+Math.round(el)+"s/"+(l+1)+"s)，推送下一章",c.bookTitle+" #"+c.chapterIndex,null)}}}}catch(_){return}
if(abt){whEt(r);return}if(!adv)return;
...
def patch_static(build_dir: str) -> None:
    ...
    html = rep(html, '        <button id="btnRescan" class="btn btn-ghost" title="重新扫描本地目录">重新扫描</button>\n', '        <button id="btnRescan" class="btn btn-ghost" title="重新扫描本地目录">重新扫描</button>\n        <button id="btnSimilarDetect" class="btn btn-ghost" title="相似有声书检测">相似检测</button>\n', "SIM-BTN")
    sim_modal_html = '''    <div class="edit-overlay" id="similarMask" hidden>
    ...
    html = rep(html, '  <script src="static/js/app.bundle', sim_modal_html + '\n  <script src="static/js/app.bundle', "SIM-MODAL")
...
async function Y(){try{let t=(await y("/api/recently-played")).items||[]'''
    ...
    sim_js_new = r'''function __esc(s){return String(s==null?"":s).replace(/[&<>"]/g,function(c){return c==="&"?"&amp;":c==="<"?"&lt;":c===">"?"&gt;":"&quot;"})}
...
function __simSelectGroup(gid,silent){var oldG=window.__sim.openGroup;window.__sim.openGroup=gid;window.__sim.openBook=null;if(oldG&&oldG!==gid)window.__sim.tplSel="";
...
function __simSelectBook(gid,bid){var oldG=window.__sim.openGroup;window.__sim.openGroup=gid;window.__sim.openBook=bid;if(oldG&&oldG!==gid)window.__sim.tplSel="";
...
function __simTplName(tp){var s="保留"+(tp.keep||[]).slice().sort(function(a,b){return a-b}).join(",");var D=(tp.del||[]).slice().sort(function(a,b){return a-b});if(D.length)s+=" · 删"+D.join(",");(tp.copies||[]).forEach(function(c){s+=" · "+__fieldName(c.field)+c.from+"→"+c.to});return s}
function __simTplSave(gid){var wf=window.__sim.wf,g=null;for(var i=0;i<wf.groups.length;i++)if(wf.groups[i].id===gid){g=wf.groups[i];break}if(!g)return;var st=wf.settings[gid]||{method:"none",keepIds:[],metaCopies:[]};if(st.method!=="delete"){alert("当前处理方式不可保存为模板\n仅「删除重复」可保存为模板");__simTplMsg("当前处理方式不可保存为模板");return}var keep=[],del=[];g.books.forEach(function(b,bi){if((st.keepIds||[]).indexOf(b.id)>=0)keep.push(bi+1);else del.push(bi+1)});if(!keep.length){alert("至少需要保留一本\n全部删除不可保存为模板");__simTplMsg("模板必须至少保留一本");return}var copies=(st.metaCopies||[]).map(function(mc){return{from:mc.from+1,to:mc.to+1,field:mc.field}});wf.templates=wf.templates||[];var tp={id:"tpl"+Date.now().toString(36)+Math.random().toString(36).slice(2,6),keep:keep,del:del,copies:copies};tp.name=__simTplName(tp);wf.templates.push(tp);window.__sim.tplSel=tp.id;__simRenderProc(gid);__simSave();__simTplMsg("模板已保存："+tp.name);u("模板已保存")}
function __simTplDel(gid){var wf=window.__sim.wf,el=document.getElementById("smTpl"),id=(el&&el.value)?el.value:(window.__sim.tplSel||"");if(!id){u("请先在下拉框选择要删除的模板");return}var nm="",ls=wf.templates||[];for(var i=0;i<ls.length;i++)if(ls[i].id===id)nm=ls[i].name||"";wf.templates=ls.filter(function(x){return x.id!==id});window.__sim.tplSel="";__simRenderProc(gid);__simSave();__simTplMsg("模板已删除："+(nm||id));u("模板已删除："+(nm||id))}
function __simTplApply(gid,id){var wf=window.__sim.wf,g=null;for(var i=0;i<wf.groups.length;i++)if(wf.groups[i].id===gid){g=wf.groups[i];break}if(!g)return;var tp=null;for(var j=0;j<(wf.templates||[]).length;j++)if(wf.templates[j].id===id){tp=wf.templates[j];break}if(!tp)return;var N=g.books.length,bad=[];if(!(tp.keep||[]).length){alert("模板无效：未指定保留的有声书");return}(tp.keep||[]).forEach(function(k){if(k>N)bad.push("要保留第"+k+"本")});(tp.copies||[]).forEach(function(c){if(c.from>N)bad.push("拷贝源第"+c.from+"本");if(c.to>N)bad.push("拷贝目标第"+c.to+"本")});if(bad.length){alert("无法应用模板\n当前组只有 "+N+" 本，缺少：\n· "+bad.join("\n· "));__simTplMsg("无法应用模板：当前组仅 "+N+" 本");return}var st=wf.settings[gid]||{method:"none",keepIds:[],metaCopies:[]};st.method="delete";st.keepIds=(tp.keep||[]).map(function(k){return g.books[k-1].id});st.metaCopies=(tp.copies||[]).map(function(c){return{from:c.from-1,to:c.to-1,field:c.field}});wf.settings[gid]=st;__simRenderProc(gid);__simSave();__simTplMsg("已应用模板："+tp.name);u("模板已应用")}
function __simTplMsg(t){try{var m=document.getElementById("smMsg");if(m)m.textContent=t}catch(_){}}function __simTplHot(n){var wf=window.__sim.wf;if(!window.__sim.open||!window.__sim.openGroup)return;var ls=wf.templates||[];if(n<1||n>ls.length){u("没有第 "+n+" 个模板（共 "+ls.length+" 个）");return}var tp=ls[n-1],gid=window.__sim.openGroup;window.__sim.tplSel=tp.id;var el=document.getElementById("smTpl");if(el)el.value=tp.id;__simTplApply(gid,tp.id)}
...
h+='<div class="sm-row"><label>处理模板</label><select id="smTpl"><option value="">— 选择模板 —</option>';(wf.templates||[]).forEach(function(tp){h+='<option value="'+__esc(tp.id)+'"'+(window.__sim.tplSel===tp.id?' selected':'')+'>'+__esc(tp.name)+'</option>'});h+='</select><button class="sm-btn sm-small" id="smSaveTpl">保存模板</button><button class="sm-btn sm-small" id="smDelTpl">删除模板</button></div>';
...
(function(se){if(se&&window.__sim.tplSel)se.value=window.__sim.tplSel})(document.getElementById("smTpl"));proc.querySelector("#smSaveTpl").onclick=function(){__simTplSave(gid)};proc.querySelector("#smDelTpl").onclick=function(){__simTplDel(gid)};(function(el){if(!el)return;el.onchange=function(){window.__sim.tplSel=el.value||"";if(!el.value)return;__simTplApply(gid,el.value)}})(document.getElementById("smTpl"));
...
__similarInit();
'''
    js = rep(js, "function __syncPlayBtns(){", sim_js_new + "function __syncPlayBtns(){", "SIM-JS")
...
async function __msSelectAll(){try{let lf=(document.getElementById("libFilter")||{}).value||"",t=await y("/api/book-ids?keyword="+encodeURIComponent(n.keyword||"")+(lf?"&libFilter="+encodeURIComponent(lf):""));(t.ids||[]).forEach(e=>window.__sel.add(e.id)),__msAfter(),u("\\u5df2\\u5168\\u9009 "+(t.ids||[]).length+" \\u672c\\uff08\\u542b\\u5176\\u4ed6\\u9875\\uff09")}catch(e){u(e.message)}}
...
let ai=document.getElementById("msAllInvert");ai&&ai.addEventListener("click",async()=>{try{let lf=(document.getElementById("libFilter")||{}).value||"",t=await y("/api/book-ids?keyword="+encodeURIComponent(n.keyword||"")+(lf?"&libFilter="+encodeURIComponent(lf):""));(t.ids||[]).forEach(function(b){window.__sel.has(b.id)?window.__sel.delete(b.id):window.__sel.add(b.id)}),__msAfter(),u("\\u5df2\\u5bf9\\u5168\\u90e8"+(t.ids||[]).length+"\\u672c\\u6267\\u884c\\u53cd\\u9009")}catch(e){u(e.message)}});
...
'          <button class="btn btn-ghost" id="btnSplitCol" title="${e.virt==="shorts"?' + _qs("拆分自动合集：选中的书归入新合集，不再随原合集合并（不删除文件）") + ':' + _qs("拆分合集：选中的书归入新合集（不删除文件）") + '}"${(e.virt==="shorts"||e.virt==="custom")?"":" hidden"}>\\u62C6\\u5206\\u5408\\u96C6</button>')
    ...
    j_chapter_helpers = (
        ...
        "function __natChapterCmp(a,b){var A=String(a.title||\"\"),B=String(b.title||\"\"),ra=A.match(/\\d+|\\D+/g)||[A],rb=B.match(/\\d+|\\D+/g)||[B],m=Math.min(ra.length,rb.length);for(var i=0;i<m;i++){var x=ra[i],y=rb[i];if(/^\\d+$/.test(x)&&/^\\d+$/.test(y)){var nx=parseInt(x,10),ny=parseInt(y,10);if(nx!==ny)return nx-ny}else if(x!==y){return x<y?-1:1}}return ra.length-rb.length}function __effChapters(book,asc){var ch=book&&book.chapters||[];if(window.__chapterOrder&&window.__chapterOrder[book.id]&&window.__chapterOrder[book.id].length){var rk=window.__chapterOrder[book.id],map={},arr=[],seen={};ch.forEach(function(c){map[c.id]=c});rk.forEach(function(id){if(map[id]){arr.push(map[id]);seen[id]=1}});ch.forEach(function(c){if(!seen[c.id])arr.push(c)});return arr}var isCol=book.virt===\"shorts\"||book.virt===\"custom\"||book.virt===\"pack\";return ch.slice().sort(function(a,b){if(isCol){return asc?__natChapterCmp(a,b):__natChapterCmp(b,a)}return asc?a.index-b.index:b.index-a.index})}\n"
    )
    ...
    j_chapter_sort_new = (';let __isCol=e.virt==="shorts"||e.virt==="custom"||e.virt==="pack";let a=[...e.chapters].sort((i,s)=>{var r=__isCol?__natChapterCmp(i,s):i.index-s.index;return n.chapterSortOrder==="asc"?r:-r});'
        ...
        'else if(window.__chapterSortPreview){a=[...e.chapters].sort((i,s)=>{var r2=__isCol?__natChapterCmp(i,s):i.index-s.index;return n.chapterSortOrder==="asc"?r2:-r2})}'
    ...
    j_csh_old = 'return ch.slice().sort(function(a,b){if(isCol){return asc?__natChapterCmp(a,b):__natChapterCmp(b,a)}return asc?a.index-b.index:b.index-a.index})}'
